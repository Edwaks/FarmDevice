...
**FarmDevice** is an Android device environment for TaskCapitan tasks. It is designed for mobile-process operators and owners of repeatable workflows and keeps devices available and tasks observable from assignment through completion.

- Public URL: https://taskcapitan.ru/
- Core entities: devices, groups, tasks, sessions, states, confirmations, and results.
- Key capabilities: device registry, grouping, task assignment, availability status, session monitoring, result recording, execution history.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. FarmDevice provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Devices, groups, tasks, sessions, states, confirmations, and results are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Device-farm operator, Requester, and Reviewer work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Device registry** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Grouping** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Task assignment** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Availability status** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Session monitoring** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Result recording** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Execution history** — a distinct part of the workflow, connected to the product model and a clear user outcome.
